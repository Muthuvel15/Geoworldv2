...
function getCountriesByContinent($continent)
{
  ...
  global $pdo;
  $query = 'SELECT c.id AS codepays, c.Code,c.Name AS pays,ci.Name AS cityname,c.Population,c.IndepYear,c.LifeExpectancy,c.GNP,c.GovernmentForm
...
function getlangue($idcountry)
{
    global $pdo;
    $query = 'SELECT l.Name
    FROM Country c, countrylanguage cl, language l
    WHERE c.id=cl.idCountry
    AND cl.IsOfficial = TRUE
    AND l.id=cl.idLanguage
    AND c.id= :id';
    $prep = $pdo->prepare($query);
...
function getCountriesByContinent1($continent)
{
  // pour utiliser la variable globale dans la fonction
  global $pdo;
...
{
    global $pdo;
    $requete ="UPDATE country set Code= :Code, Population= :Population, IndepYear= :IndepYear, GNP= :GNP, GovernmentForm= :GovernmentForm  where id = :id";
...
{
        global $pdo;
        $requete = "SELECT c.id, c.code,c.Continent, c.Name AS Pays, ci.Name AS Capital, c.Population, c.IndepYear, c.LifeExpectancy, c.GNP, c.GovernmentForm  
        FROM country c, city ci  
        WHERE c.Capital=ci.id 
        AND c.id = :id";
        $prep = $pdo->prepare($requete);
